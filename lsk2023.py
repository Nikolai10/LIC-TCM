...
This is a reimplementation of LIC_TCM published in:
J. Liu and H. Sun and J. Katto:
"Learned Image Compression with Mixed Transformer-CNN Architectures"
Conference on Computer Vision and Pattern Recognition (CVPR), 2023
https://arxiv.org/pdf/2303.14978
...
from arch_ops import ConvTransBlock, conv3x3, ResidualBlockWithStride, ResidualBlockUpsample, subpel_conv3x3, conv, \
    SWAtten
from config import ConfigLSK as cfg_lsk
...
class AnalysisTransform(tf.keras.Sequential):
    ...
    def __init__(self):
        ...
        dim = cfg_lsk.N
        head_dim = cfg_lsk.head_dim
        window_size = cfg_lsk.window_size
        config = cfg_lsk.config
        dpr = cfg_lsk.drop_path_rate
        N = cfg_lsk.N
        M = cfg_lsk.M

        data_norm = [tf.keras.layers.Lambda(lambda x: x / 255.)]

        # m_down1
        m_down1 = [ConvTransBlock(dim, dim, head_dim[0], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                   range(config[0])] + [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]
        # m_down2
        m_down2 = [ConvTransBlock(dim, dim, head_dim[1], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                   range(config[1])] + [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]
        # m_down3
        m_down3 = [ConvTransBlock(dim, dim, head_dim[2], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                   range(config[2])] + [conv3x3(2 * N, M, stride=2)]

        layers = data_norm + [ResidualBlockWithStride(3, 2 * N, 2)] + m_down1 + m_down2 + m_down3
        for layer in layers:
            self.add(layer)
...
class SynthesisTransform(tf.keras.Sequential):
    ...
    def __init__(self):
        ...
        # m_up1
        m_up1 = [ConvTransBlock(dim, dim, head_dim[3], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                 range(config[3])] + [ResidualBlockUpsample(2 * N, 2 * N, 2)]
        # m_up2
        m_up2 = [ConvTransBlock(dim, dim, head_dim[4], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                 range(config[4])] + [ResidualBlockUpsample(2 * N, 2 * N, 2)]
        # m_up3
        m_up3 = [ConvTransBlock(dim, dim, head_dim[5], window_size, dpr, 'W' if not i % 2 else 'SW') for i in
                 range(config[5])] + [subpel_conv3x3(2 * N, 3, 2)]

        data_denorm = [tf.keras.layers.Lambda(lambda x: x * 255.)]

        layers = [ResidualBlockUpsample(M, 2 * N, 2)] + m_up1 + m_up2 + m_up3 + data_denorm
        for layer in layers:
            self.add(layer)
...
class HyperAnalysisTransform(tf.keras.Sequential):
    ...
    def __init__(self, hyperprior_depth):
        ...
        ha_down1 = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i % 2 else 'SW') for i in range(config[0])] + [
            conv3x3(2 * N, 192, stride=2)]

        layers = [ResidualBlockWithStride(320, 2 * N, 2)] + ha_down1
        for layer in layers:
            self.add(layer)
...
class HyperSynthesisTransform(tf.keras.Sequential):
    ...
    def __init__(self):
        ...
        hs_up = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i % 2 else 'SW') for i in range(config[3])] + [
            subpel_conv3x3(2 * N, 320, 2)]
        layers = [ResidualBlockUpsample(192, 2 * N, 2)] + hs_up
...
class SliceTransform(tf.keras.layers.Layer):
    ...
    def __init__(self, latent_depth, num_slices):
        ...
        # size of the corresponding slice. If we have 5 slices and a bottleneck
        # with 320 channels, the output is 320 / 5 = 64 channels.
        ...
        self.transform = tf.keras.Sequential([
            conv(224, stride=1, kernel_size=3),
            tf.keras.layers.Lambda(lambda x: tf.nn.gelu(x)),
            conv(128, stride=1, kernel_size=3),
            tf.keras.layers.Lambda(lambda x: tf.nn.gelu(x)),
            conv((320 // num_slices), stride=1, kernel_size=3),
        ])
...
class SWAttenTransform(tf.keras.layers.Layer):
    """SWAtten transform."""
    ...
    def __init__(self, latent_depth, num_slices, index):
        ...
        self.transform = tf.keras.Sequential([
            SWAtten((320 + (320 // num_slices) * min(i, 5)), (320 + (320 // num_slices) * min(i, 5)), 16, window_size,
                    0, inter_dim=128)
        ])
...
class LSK2023Model(tf.keras.Model):
    """Main model class."""

    def __init__(self, lmbda, latent_depth, hyperprior_depth, num_scales, scale_min, scale_max):
        super().__init__()
        self.lmbda = lmbda
        self.num_scales = num_scales
        self.num_slices = cfg_lsk.num_slices
        self.max_support_slices = cfg_lsk.max_support_slices
        ...
        self.cc_mean_transforms = [
            SliceTransform(latent_depth, self.num_slices) for _ in range(self.num_slices)]
        self.cc_scale_transforms = [
            SliceTransform(latent_depth, self.num_slices) for _ in range(self.num_slices)]
        self.lrp_transforms = [
            SliceTransform(latent_depth, self.num_slices) for _ in range(self.num_slices)]
        self.atten_mean = [
            SWAttenTransform(latent_depth, self.num_slices, i) for i in range(self.num_slices)]
        self.atten_scale = [
            SWAttenTransform(latent_depth, self.num_slices, i) for i in range(self.num_slices)]
        ...
        self.decompress = tf.function(
            input_signature=3 * [tf.TensorSpec(shape=(2,), dtype=tf.int32)] +
                            (self.num_slices + 1) * [tf.TensorSpec(shape=(1,), dtype=tf.string)]
        )(self.decompress)
    ...
    def call(self, x, training):
        ...
        loss = total_bpp + self.lmbda * distortion_loss

        return loss, total_bpp, distortion_loss
...
def train(args):
    ...
    model = LSK2023Model(
        args.lmbda, args.latent_depth, args.hyperprior_depth,
        args.num_scales, args.scale_min, args.scale_max)
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=args.lr, global_clipnorm=1.0),
    )
